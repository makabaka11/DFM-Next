...
impl FrameCompletionState {
    ...
    pub(crate) fn completed_sequence(&self) -> u64 {
        self.completed_sequence.load(Ordering::Acquire)
    }
...
impl GpuCompletionDriver {
    pub(crate) fn start(device: CompletionArc<wgpu::Device>) -> Result<Self, String> {
        ...
        thread::Builder::new()
            .name("dfm-gpu-completion".to_string())
            .spawn(move || completion_worker(device, worker_wake))
            .map_err(|err| format!("spawn dfm GPU completion thread failed: {err}"))?;
        Ok(Self { wake })
    }

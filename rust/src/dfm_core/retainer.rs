...
/// Uses a track + compaction approach for correct pre-computed layout.
///
...
impl TrackData {
    ...
    /// An entry is expired when its full duration has elapsed
...
///   Within the overflow zone, find the entry whose right edge is closest to
///   exiting the screen — that entry is replaced in place by the new danmaku.
///   Additionally, any OTHER entry on the same victim track whose right edge
///   is already past `view_width * EXIT_PROXIMITY_RATIO` (i.e. mostly off-screen
///   and about to exit naturally anyway) is also evicted. The remaining
///   mid-screen entries on the victim track stay and continue scrolling — the
///   new entry briefly overlaps them, matching Bilibili's "overwriteInsert"
///   semantics ("覆盖插入" — overlap is acceptable; mass disappearance is not).
///
///   The double design point — single-victim placement PLUS proximity-evict —
///   is the sweet spot found by trial:
///     - Clearing the WHOLE victim track strobed N danmaku per overflow burst.
///     - Replacing ONLY a single entry left every old entry alive, doubling
///       on-screen scroll count under load and starving the GPU's frame budget
///       (perceived as scroll judder).
///     - Evicting one + proximity-evict trims the renderer load back down
///       without making any visually-prominent entry disappear, because
///       proximity-evicted entries were already a few frames from exiting.
///
...
fn select_scroll_track(
    ...
    let mut best_entry_idx: usize = 0;
    ...
    for i in overwrite_start..track_count {
        ...
        let mut track_min_entry_idx: usize = 0;
        for (entry_idx, existing) in track_data.tracks[i].iter().enumerate() {
            if scroll_entries_collide(new_entry, existing, view_width) {
                collides = true;
            }
        ...
        // the single entry (across ALL tracks in the sacrifice zone) whose
        // right edge is closest to exiting — that one entry will be displaced.
        if track_min_right < min_right_edge {
            min_right_edge = track_min_right;
            best_track = i;
            best_entry_idx = track_min_entry_idx;
        }
    }

    // Phase 3: All tracks collide — overwrite the best candidate.
    //
    // We do TWO things in one shot:
    //   (a) Replace the single entry with the smallest right edge (closest to
    //       naturally exiting) with the new danmaku — this gives the new entry
    //       a place to start without strobing any visibly-prominent danmaku.
    //   (b) Also evict any OTHER entries on the same victim track whose right
    //       edge has already fallen below EXIT_PROXIMITY_PX — those entries
    //       are a handful of frames from exiting the screen anyway, so their
    //       removal is visually indistinguishable from the natural exit and
    //       lets us shed render load.
    //
    // (a) alone (single-entry replace) keeps every old entry alive under
    // sustained overflow, doubling on-screen scroll count and starving the
    // GPU's frame budget — visible as scroll judder. (b) alone (full track
    // clear) strobes N items per burst.  Together they give the same
    // smoothness as the original clear-track behaviour without the strobe.
    //
    // EXIT_PROXIMITY_RATIO is conservative on purpose: a danmaku at this much
    // remaining right-edge is genuinely on its way out, not "still
    // mid-screen".
    const EXIT_PROXIMITY_RATIO: f32 = 0.15;
    let exit_proximity_px = view_width * EXIT_PROXIMITY_RATIO;

    if is_me && track_count > 0 {
        // For user's own danmaku, force placement on track 0 by displacing only the
        // closest-to-exit entry there, plus the same proximity-evict pass.
        if !track_data.tracks[0].is_empty() {
            let mut victim_idx = 0;
            let mut victim_right = f32::MAX;
            for (i, e) in track_data.tracks[0].iter().enumerate() {
                let r = entry_right_edge_at(e, new_entry.time_ms, view_width);
                if r < victim_right {
                    victim_right = r;
                    victim_idx = i;
                }
            }
            let mut displaced: DisplacedIndices = SmallVec::new();
            // Proximity-evict pass: collect indices of also-about-to-exit
            // entries BEFORE we mutate the track. Skip the victim slot
            // itself — it'll get replaced separately.
            let extra_evict: SmallVec<[usize; 4]> = track_data.tracks[0]
                .iter()
                .enumerate()
                .filter(|(i, e)| {
                    *i != victim_idx
                        && entry_right_edge_at(e, new_entry.time_ms, view_width)
                            < exit_proximity_px
                })
                .map(|(i, _)| i)
                .collect();
            for &i in &extra_evict {
                displaced.push(track_data.tracks[0][i].danmaku_index);
            }
            displaced.push(track_data.tracks[0][victim_idx].danmaku_index);
            // Remove from highest index first to keep lower indices stable;
            // then replace the victim slot. victim_idx adjusts as evictions
            // ahead of it shift entries down.
            let mut adjusted_victim = victim_idx;
            let mut sorted_evict: SmallVec<[usize; 4]> = extra_evict.clone();
            sorted_evict.sort_unstable_by(|a, b| b.cmp(a));
            for &i in &sorted_evict {
                track_data.tracks[0].remove(i);
                if i < adjusted_victim {
                    adjusted_victim -= 1;
                }
            }
            track_data.tracks[0][adjusted_victim] = new_entry.clone();
            return Some((0, displaced));
        } else {
            track_data.tracks[0].push(new_entry.clone());
            return Some((0, SmallVec::new()));
        }
    }

    if min_right_edge < f32::MAX {
        let mut displaced: DisplacedIndices = SmallVec::new();
        // Proximity-evict on the victim track: any entry that's already mostly
        // off-screen joins the victim in the displaced list.
        let extra_evict: SmallVec<[usize; 4]> = track_data.tracks[best_track]
            .iter()
            .enumerate()
            .filter(|(i, e)| {
                *i != best_entry_idx
            ...
            .collect();
        for &i in &extra_evict {
            displaced.push(track_data.tracks[best_track][i].danmaku_index);
        }
        displaced.push(track_data.tracks[best_track][best_entry_idx].danmaku_index);

        // Remove evicted entries from highest index first to keep lower indices stable,
        // tracking how best_entry_idx shifts as earlier slots are removed.
        let mut adjusted_victim = best_entry_idx;
        let mut sorted_evict: SmallVec<[usize; 4]> = extra_evict.clone();
        sorted_evict.sort_unstable_by(|a, b| b.cmp(a));
        for &i in &sorted_evict {
            track_data.tracks[best_track].remove(i);
            if i < adjusted_victim {
                adjusted_victim -= 1;
            }
        }
        // Replace the victim slot in place with the new entry.
        track_data.tracks[best_track][adjusted_victim] = new_entry.clone();
...
/// Clears entire tracks when the item ends.
...
mod tests {
    ...
    fn test_overflow_queues_item() {
        ...
        // Second item: track still occupied → dropped.
    ...
    fn test_chain_queue_fixed_items() {
        ...
        // First item gets placed; subsequent items are dropped.

...
///
/// Output format is compatible with GPU rendering pipelines (JSON-based
/// FrameItemPayload), allowing direct reuse of the MSDF text rendering
/// engine for on-screen display.
...
pub fn dfm_plus_prepare_layout(
    mut request: DfmPlusPrepareRequest,
...
pub fn dfm_plus_prepare_layout_full(
    ...
    // Sort by time_seconds so the retainer processes danmaku in chronological
    // order. Without sorting, items that arrive out-of-order can cause the
    // compact() pass to remove entries that haven't actually expired yet,
    // leading to incorrect collision detection and different overwriteInsert
    // victim selection when configure() is re-run.

...
@_silgen_name("dfm_engine_create")
private func dfm_engine_create(_ width: UInt32, _ height: UInt32) -> UInt64

@_silgen_name("dfm_engine_get_mtl_device")
private func dfm_engine_get_mtl_device(_ engineHandle: UInt64) -> UnsafeMutableRawPointer?

@_silgen_name("dfm_engine_attach_present_texture")
private func dfm_engine_attach_present_texture(
...
@_silgen_name("dfm_engine_dispose")
private func dfm_engine_dispose(_ engineHandle: UInt64)

@_silgen_name("dfm_engine_poll_frame_ready")
private func dfm_engine_poll_frame_ready(_ engineHandle: UInt64) -> Bool

@_silgen_name("dfm_engine_set_frame")
private func dfm_engine_set_frame(
  _ engineHandle: UInt64,
...
@_silgen_name("dfm_engine_resize")
private func dfm_engine_resize(_ engineHandle: UInt64, _ width: UInt32, _ height: UInt32) -> UInt8

@_silgen_name("dfm_engine_reset_scene")
private func dfm_engine_reset_scene(_ engineHandle: UInt64) -> UInt8

private final class DfmTexture: NSObject, FlutterTexture {
...
private final class DfmSurfaceState {
  var surfaceId: String
  var texture: DfmTexture
  var textureId: Int64?
  ...
  init(surfaceId: String, width: Int, height: Int) {
    ...
    self.texture = DfmTexture(width: width, height: height)
    self.textureId = nil
...
private func dfmDisplayLinkCallback(
  ...
  autoreleasepool {
    let plugin = Unmanaged<DfmPlusNativePlugin>.fromOpaque(displayLinkContext).takeUnretainedValue()
    plugin.onDisplayLinkTick()
  }
  return kCVReturnSuccess
}

public final class DfmPlusNativePlugin: NSObject, FlutterPlugin {
  private static let channelName = "dfm_plus/texture"
  private static var didRegister = false

  private let textureRegistry: FlutterTextureRegistry
  private let stateLock = NSLock()
  private var surfaces: [String: DfmSurfaceState] = [:]
  private var displayLink: CVDisplayLink?
  private let initQueue = DispatchQueue(label: "dfm.plus.engine-init", qos: .userInitiated)
  ...
  public static func register(with registrar: FlutterPluginRegistrar) {
    ...
    let instance = DfmPlusNativePlugin(textureRegistry: registrar.textures)
  ...
  private func getTextureInfo(
    ...
    stateLock.lock()
    let entry: DfmSurfaceState
    if let existing = surfaces[surfaceId] {
      entry = existing
    } else {
      let created = DfmSurfaceState(surfaceId: surfaceId, width: width, height: height)
    ...
    initQueue.async { [weak self] in
      ...
      let targetTexture = recreateTexture ? DfmTexture(width: width, height: height) : oldTexture
      ...
      if handle == 0 {
        handle = dfm_engine_create(UInt32(resolvedWidth), UInt32(resolvedHeight))
        engineCreated = true
      } else if needsResize {
        let resizeOk = dfm_engine_resize(handle, UInt32(resolvedWidth), UInt32(resolvedHeight)) != 0
        if !resizeOk {
          dfm_engine_dispose(handle)
          handle = dfm_engine_create(UInt32(resolvedWidth), UInt32(resolvedHeight))
          engineCreated = true
      ...
      guard handle != 0 else {
        DispatchQueue.main.async {
          self.completePendingTextureInfoRequests(
            surfaceId: surfaceId,
            response: FlutterError(code: "engine_create_failed", message: "dfm_engine_create returned 0", details: nil)
          )
        }
        return
      }

      guard let devicePtr = dfm_engine_get_mtl_device(handle) else {
        DispatchQueue.main.async {
          self.completePendingTextureInfoRequests(
            surfaceId: surfaceId,
            response: FlutterError(
              code: "engine_get_mtl_device_failed",
              message: "dfm_engine_get_mtl_device returned null",
              details: nil
            )
          )
        }
      ...
      dfm_engine_attach_present_texture(
        handle,
        texturePtr,
    ...
    let ok = frameJson.withCString { ptr in
      customFontFamily.withCString { familyPtr in
        customFontFilePath.withCString { filePtr in
          dfm_engine_set_frame(handle, ptr, fontSize, outlineWidth, shadowStyle, opacity, familyPtr, filePtr) != 0
        }
      }
    }
    result(ok)
  ...
  private func disposeSurface(surfaceId: String, result: @escaping FlutterResult) {
    ...
    if let entry = removed {
      ...
      if entry.engineHandle != 0 {
        initQueue.async {
          dfm_engine_dispose(entry.engineHandle)
        }
      }
  ...
  fileprivate func onDisplayLinkTick() {
    ...
    for (handle, textureId) in entries {
      let ready = dfm_engine_poll_frame_ready(handle)
      if ready {

...
int64_t dfm_engine_prefetch_pending(uint64_t handle);
uint8_t dfm_engine_set_frame_ready_event(uint64_t handle,
                                           uintptr_t event_handle);
uint8_t dfm_engine_create_dxgi_shared_texture(uint64_t handle,
                                                uint32_t width,
                                                uint32_t height,
...
struct DfmPlusNativePlugin::TextureBinding {
...
struct DfmPlusNativePlugin::SurfaceState {
...
void DfmPlusNativePlugin::RegisterWithRegistrar(
    flutter::PluginRegistrarWindows* registrar) {
  auto plugin = std::make_unique<DfmPlusNativePlugin>(registrar);
  registrar->AddPlugin(std::move(plugin));
}

DfmPlusNativePlugin::DfmPlusNativePlugin(
    flutter::PluginRegistrarWindows* registrar)
...
DfmPlusNativePlugin::~DfmPlusNativePlugin() {
  ...
  for (auto& surface : removed_surfaces) {
    if (surface->engine_handle != 0) {
      dfm_engine_set_frame_ready_event(surface->engine_handle, 0);
    }
    ReleaseTexture(surface.get());
    if (surface->engine_handle != 0) {
      dfm_engine_dispose(surface->engine_handle);
    }
  }
...
void DfmPlusNativePlugin::HandleMethodCall(
  ...
  if (method_call.method_name() == "getTextureInfo") {
    ...
    if (it == surfaces_.end()) {
      ...
      if (created->engine_handle == 0) {
        result->Error("engine_create_failed", "dfm_engine_create returned 0");
        return;
      }
      is_new_engine = true;
    ...
    if (state->engine_handle == 0) {
      state->engine_handle = dfm_engine_create(width, height);
      if (state->engine_handle == 0) {
        result->Error("engine_create_failed", "dfm_engine_create returned 0");
        return;
      }
      ...
      const auto ok = dfm_engine_resize(state->engine_handle, width, height);
      if (ok == 0) {
    ...
    if (dfm_engine_set_frame_ready_event(
            state->engine_handle,
  ...
  if (method_call.method_name() == "setFrame") {
    ...
    const uint8_t ok = dfm_engine_set_frame(handle, frame_json->c_str(),
  ...
  if (method_call.method_name() == "getDfmPrewarmState") {
    ...
    const int64_t pending_glyphs = dfm_engine_prefetch_pending(handle);
    if (pending_glyphs < 0) {
...
void DfmPlusNativePlugin::DisposeSurface(const std::string& surface_id) {
  ...
  if (removed->engine_handle != 0) {
    dfm_engine_set_frame_ready_event(removed->engine_handle, 0);
  }
  if (removed->texture_id >= 0) {
    ReleaseTexture(removed.get());
  }
  if (removed->engine_handle != 0) {
    dfm_engine_dispose(removed->engine_handle);
  }
}

void DfmPlusNativePlugin::ReleaseTexture(SurfaceState* state) {
  if (!state) {
    return;
  }
...
void DfmPlusNativePlugin::EnsureTickThreadRunning() {
...
void DfmPlusNativePlugin::StopTickThread() {
  if (!tick_running_.load()) {
    return;
  }
